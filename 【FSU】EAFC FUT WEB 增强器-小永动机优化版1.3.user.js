...
(function () {
    ...
    function futweb() {
        ...
        events.openFutbinPlayerUrl = async(e) => {
            ...
            let playerUrl;
            try {
                playerUrl = await events.getFutbinUrl(`https://www.futbin.org/futbin/api/${info.base.year}/searchPlayersByName?playername=${n}&year=${info.base.year}`);
            }catch(error) {
                return;
            }
            let fi = "";
        ...
        //26.03 一键替换所有假想球员为满足阵容要求球员（基于SBCSetMeetsPlayers逻辑重写）
        events.SBCSetAllConceptPlayers = async(e) => {
            ...
            const isSbcCandidate = (candidate) => {
                if (candidate && typeof candidate.isSbc === "boolean") {
                    return candidate.isSbc;
                }
                return availableSbcStoragePlayers.includes(getCandidatePlayer(candidate));
            };
            ...
            const compareQualifiedPlayers = (candidateA, candidateB, prioritizeChemistry, sbcPriority) => {
                ...
                if (sbcPriority) {
                    let aIsSbc = isSbcCandidate(candidateA);
                    let bIsSbc = isSbcCandidate(candidateB);
                    if (aIsSbc !== bIsSbc) {
                        return aIsSbc ? -1 : 1;
                    }
                }
            ...
            const tryDowngradeFinalSquad = (players) => {
                ...
                const compareDowngradeCandidates = (a, b) => {
                    if (sbcPriority) {
                        let aIsSbc = availableSbcStoragePlayers.includes(a);
                        let bIsSbc = availableSbcStoragePlayers.includes(b);
                        if (aIsSbc !== bIsSbc) {
                            return aIsSbc ? -1 : 1;
                        }
                    }
            ...
            console.log("开始循环替换假想球员，初始已使用球员ID:", Array.from(usedPlayerIds));
            console.log("初始已使用球员definitionId:", Array.from(usedPlayerDefIds));
            ...
            while(iteration < maxIterations) {
                ...
                console.log(`第 ${iteration} 轮发现 ${conceptPlayerIndices.length} 个假想球员`);

                // 如果没有假想球员了，退出循环
                if(conceptPlayerIndices.length === 0) {
                    console.log("没有更多假想球员需要替换，退出循环");
                    break;
                }
                ...
                let replacedCount = 0;
                let successfulReplacements = []; // 记录成功的替换
                ...
                for(let index of conceptPlayerIndices) {
                    ...
                    // 合并所有可用球员（SBC仓库球员 + 短名单球员）
                    let allAvailablePlayers = [...availableSbcStoragePlayers, ...availableShortlistPlayers];

                    // 过滤掉已使用的球员（按ID和definitionId双重过滤）
                    allAvailablePlayers = allAvailablePlayers.filter(player => {
                    ...
                    console.log(`位置 ${index} 可用球员数量:`, allAvailablePlayers.length);
                    ...
                    if (targetRating > 0) {
                        allAvailablePlayers.sort((a, b) => {
                            ...
                            // 如果都低于或等于目标评分，选择评分较低的（节省资源）
                            if (aRating <= targetRating && bRating <= targetRating) {
                                return aRating - bRating;
                            }

                            // 如果都高于目标评分，选择最接近的（避免过度浪费）
                            if (aRating > targetRating && bRating > targetRating) {
                                return (aRating - targetRating) - (bRating - targetRating);
                            }
                        ...
                        try {
                            ...
                            if(meetsRequirements) {
                                ...
                                qualifiedPlayers.push({
                                    player: player,
                                    isSbc: availableSbcStoragePlayers.includes(player),
                    ...
                    console.log(`位置 ${index} 满足条件的球员数量:`, qualifiedPlayers.length);
                    ...
                    if(foundReplacement && bestReplacement) {
                        console.log(`位置 ${index} 选择球员: ${bestReplacement.name || bestReplacement._staticData?.name || 'Unknown'}(ID:${bestReplacement.id}, DefID:${bestReplacement.definitionId})`);
                        ...
                        console.log(`位置 ${index} 替换完成，已使用球员ID:`, Array.from(usedPlayerIds));
                        console.log(`已使用球员definitionId:`, Array.from(usedPlayerDefIds));

                        // 立即应用替换到虚拟阵容，确保后续测试基于最新阵容
                        newChallenge.squad.setPlayers(tempList);
                    } else {
                        console.log(`位置 ${index} 未找到合适的替换球员，保留原假想球员`);
                        // 如果没有找到合适的替换球员，确保该位置有球员（保留原假想球员）
                        tempList[index] = originalPlayer;
                    }
                }

                console.log(`第 ${iteration} 轮替换完成，替换了 ${replacedCount} 个假想球员`);

                // 如果本轮没有替换任何球员，说明已经无法继续替换，退出循环
                if(replacedCount === 0) {
                    console.log("本轮没有替换任何球员，无法继续优化，退出循环");
                    break;
                }
            ...
            console.log(`\n=== 替换完成，总共进行了 ${iteration} 轮，替换了 ${totalReplacedCount} 个假想球员 ===`);
            ...
            if(totalReplacedCount > 0) {
                ...
                if (!hasConceptPlayers) {
                    ...
                    if (downgradeState.changed) {
                        events.saveSquad(challenge, squad, downgradeState.players, []);
                    }
                }
                // 最终阵容验证
                const finalSquadPlayers = squad.getPlayers();
                console.log(`最终阵容验证: ${finalSquadPlayers.length}名球员`);
        ...
        events.optimizeSquadChemistry = async (challenge, squad, targetChemistry = 33, lockedIndices = []) => {
            try {
                {
                    let existingChemistry = squad.getChemistry();
                    if (existingChemistry >= targetChemistry) {
                        return;
                    }

                    let currentPlayers = squad.getFieldPlayers().map(i => i.item);
                    let optimizedState = events.findBestChemistryArrangement(squad, currentPlayers, {
                        targetChemistry: targetChemistry,
                        lockedIndices: lockedIndices
                    });

                    if (optimizedState.chemistry > existingChemistry) {
                        if (!info.base.oneClickReplace) {
                            events.saveSquad(challenge, squad, optimizedState.players, []);
                        } else {
                            squad.setPlayers(optimizedState.players, true);
                        }
                        console.log(`榛樺搴︿紭鍖栧畬鎴愶細${existingChemistry} 鈫?${optimizedState.chemistry}`);
                    }
                    return;
                }
                let currentPlayers = squad.getFieldPlayers().map(i => i.item);
                ...
                // 获取当前默契度
                let currentChem = squad.getChemistry();
                ...
                let bestChemistry = currentChem;

                // 全面的优化策略：尝试交换所有位置球员
                // 生成所有可能的位置排列组合（限制组合数量避免性能问题）
                let maxIterations = Math.min(100, factorial(currentPlayers.length));
                let testedCombinations = new Set();

                // 先测试当前阵容
                testedCombinations.add(currentPlayers.map(p => p.id).join(','));

                // 随机交换位置进行测试
                for (let iter = 0; iter < maxIterations; iter++) {
                    let testPlayers = [...currentPlayers];

                    // 随机交换多个位置
                    for (let swapCount = 0; swapCount < Math.min(3, currentPlayers.length / 2); swapCount++) {
                        let i = Math.floor(Math.random() * currentPlayers.length);
                        let j = Math.floor(Math.random() * currentPlayers.length);
                        if (i !== j) {
                            [testPlayers[i], testPlayers[j]] = [testPlayers[j], testPlayers[i]];
                        }
                    }
                    ...
                    // 检查是否已经测试过这个组合
                    let combinationKey = testPlayers.map(p => p.id).join(',');
                    ...
                    // 计算新阵容的默契度
                    let chemResult = squad.chemCalculator.calculate(formation, testPlayers, manager);
                    let newChemistry = chemResult.chemistry;

                    // 如果默契度更高，更新最佳阵容
                    if (newChemistry > bestChemistry) {
                        bestChemistry = newChemistry;
                ...
                // 如果找到了更好的阵容，应用它
                if (bestChemistry > currentChem) {
                    // 一键替换时跳过保存操作以避免额外的返回动作
                    if (!info.base.oneClickReplace) {
                        events.saveSquad(challenge, squad, bestPlayers, []);
                    } else {
                        // 直接设置球员而不保存
                        squad.setPlayers(bestPlayers, true);
                    }
                    console.log(`默契度优化完成：${currentChem} → ${bestChemistry}`);
                }

            } catch (error) {
                console.warn("默契度优化失败:", error);
                // 优化失败不影响主要功能，继续执行
            }
        }
        ...
        UTClubSearchResultsView.prototype.setItemsWithChemDiff = function(t,a,s,l,c) {
            ...
            _.map(t,(player,index) => {
                let iconName = "";
                let className = "";
                if(this.activeSquad && !(this.activeSquad.containsItem(player,!0))){
                    if(player.concept){
        ...
        UTSBCSquadDetailPanelView.prototype.render = function(e, t, i, o) {
            ...
            if(!this._fsuBIB){
                ...
                // 使用与SBC方案填充按钮一致的样式
                Object.assign(this._fsuBIB.__root.style, {
                    width: 'calc(100% - 1rem)',
                    marginTop: '.675rem',
                    margin: '.5rem auto'
                });
                ...
                // 更安全的DOM插入逻辑
                try {
                    if(this._challengeDetails.__root && this._challengeDetails.__description){
                        this._challengeDetails.__root.insertBefore(this._fsuBIB.__root, this._challengeDetails.__description.nextSibling);
                    } else if(this._challengeDetails.__root){
                        // 如果description不存在，直接插入到root的末尾
                        this._challengeDetails.__root.appendChild(this._fsuBIB.__root);
                    }
                } catch(error) {
                    console.error("批量购买按钮插入失败:", error);
                    // 备用插入位置
                    if(rewardElement){
                        rewardElement.appendChild(this._fsuBIB.__root);
                    }
                }
            }
            ...
            if(e.squad){
                let conceptList = _.filter(e.squad.getPlayers(), slot => {
                    return slot.item && slot.item.concept;
                });
                if(conceptList.length > 0){
                    console.log("发现concept球员，显示批量购买按钮，数量:", conceptList.length);
                    // 修复手机端按钮显示问题：移除控制器判断条件
                    this._fsuBIB.show();
                } else {
                    console.log("未发现concept球员，隐藏批量购买按钮");
                }
            } else {
                console.log("SBC squad不存在，隐藏批量购买按钮");
            }
            ...
            //26.03 添加一键替换按钮到阵容填充按钮后面
            if(!this._fsuOneClickReplace && info.set.sbc_template && e.squad){
                let b = events.createButton(
                    new UTStandardButtonControl(),
                    "一键替换所有假想球员",
                    () => {
                        // 确保一键替换按钮没有手机端多余的返回动作
                        info.base.oneClickReplace = true;
                        events.SBCSetAllConceptPlayers(b);
                        setTimeout(() => {
                            info.base.oneClickReplace = false;
                        }, 2000);
                    },
                    "call-to-action"
                )
                b._parent = e;
                this._fsuOneClickReplace = b;
                // 使样式与_fsuSquad按钮保持一致
                this._fsuOneClickReplace.addClass("mini");

                // 将一键替换按钮插入到阵容填充按钮后面
                if(this._fsuSquad && this._fsuSquad.__root){
                    this._fsuSquad.__root.after(this._fsuOneClickReplace.__root);
                }

                // 添加优先选择SBC仓库球员的选项
                let sbcPriorityOption = document.createElement("div");
                sbcPriorityOption.className = "ut-toggle-cell-view";
                sbcPriorityOption.innerHTML = `
                    <div class="ut-toggle-cell-view__label">优先选择SBC仓库球员（低分优先）</div>
                    <div class="ut-toggle-cell-view__toggle">
                        <input type="checkbox" id="fsu_sbc_priority_option">
                        <label for="fsu_sbc_priority_option"></label>
                    </div>
                `;

                // 获取复选框元素
                const checkbox = sbcPriorityOption.querySelector("#fsu_sbc_priority_option");

                // 设置初始状态
                let savedOption = GM_getValue("fsu_sbc_priority_option", true);
                checkbox.checked = !!savedOption;

                // 设置样式
                Object.assign(sbcPriorityOption.style, {
                    width: 'calc(100% - 1rem)',
                    margin: '0.25rem auto',
                    fontSize: '0.8rem',
                    color: '#ccc',
                ...
                // 设置标签样式
                const label = sbcPriorityOption.querySelector(".ut-toggle-cell-view__label");
                ...
                // 设置切换按钮样式
                const toggle = sbcPriorityOption.querySelector(".ut-toggle-cell-view__toggle");
                ...
                // 设置标签样式（作为切换按钮）
                const toggleLabel = sbcPriorityOption.querySelector("label[for='fsu_sbc_priority_option']");
                Object.assign(toggleLabel.style, {
                    display: 'block',
                    width: '40px',
                    height: '20px',
                    backgroundColor: checkbox.checked ? '#00ff1aff' : '#ccc',
                    borderRadius: '10px',
                    position: 'relative',
                    cursor: 'pointer',
                ...
                dot.style.cssText = `
                    position: absolute;
                    left: ${checkbox.checked ? '20px' : '0'}px;
                    width: 20px;
                    height: 20px;
                ...
                // 确保初始状态正确显示
                setTimeout(() => {
                    dot.style.left = checkbox.checked ? '20px' : '0';
                    toggleLabel.style.backgroundColor = checkbox.checked ? '#00ff1aff' : '#ccc';
                }, 10);

                // 统一的事件处理函数
                const updateToggleState = (checked) => {
                    GM_setValue("fsu_sbc_priority_option", checked);
                    console.log("SBC优先级选项已保存：", checked);
                    toggleLabel.style.backgroundColor = checked ? '#00ff1aff' : '#ccc';
                    dot.style.left = checked ? '20px' : '0';
                };

                // 移除label的for属性以避免自动触发checkbox事件
                toggleLabel.removeAttribute('for');

                // 清除所有现有的事件监听器
                checkbox.removeEventListener("change", updateToggleState);

                // 统一的事件处理 - 只在toggleLabel上绑定点击事件
                ...
                // 为toggleLabel添加点击事件（支持手机端和PC端）
                toggleLabel.addEventListener("click", handleToggleClick);
                toggleLabel.addEventListener("touchstart", handleToggleClick);

                // 将选项移动到sbc-button-container中的清除阵容按钮上方
                const sbcButtonContainer = this.__root.querySelector(".sbc-button-container");
                if(sbcButtonContainer){
                    const clearSquadButton = sbcButtonContainer.querySelector("button.btn-standard");
                    if(clearSquadButton && clearSquadButton.textContent.includes("清除阵容")){
                        sbcButtonContainer.insertBefore(sbcPriorityOption, clearSquadButton);
                    }else{
                        sbcButtonContainer.appendChild(sbcPriorityOption);
                    }
                }else{
                    rewardElement.appendChild(sbcPriorityOption);
                }
            }
        ...
        events.getTemplate = async function(e,type,sId){
            ...
            if(_.size(_.filter(resultSquad,"concept")) && !isPhone()){
                let right = cntlr.right ? cntlr.right() : null;
                let view = right && right.getView ? right.getView() : null;
                if(view && view._fsuBIB){
                    view._fsuBIB.show();
                }
            }
        }
        ...
        events.refreshAutoSbcButton = (controller) => {
            ...
            if(button){
                button.setText(info.autoSbc.running ? "停止循环" : "循环完成");
            }
        };
        ...
        events.autoSbcRefreshView = async(controller) => {
            ...
            try{
                ...
                await events.autoSbcDelay(1200);
        ...
        events.startAutoSbcLoop = async(controller) => {
            ...
            try{
                while(info.autoSbc.running){
                    ...
                    let nextChallenge = await events.findNextIncompleteChallenge(controller, setEntity);
                    if(!nextChallenge){
                        let remainingChallenges = await events.autoSbcRequestChallenges(controller, setEntity);
                        let incompleteChallenges = _.filter(remainingChallenges, item => !(item.isCompleted && item.isCompleted()));
                        if(incompleteChallenges.length){
                            ...
                            await events.autoSbcDelay(1500);
                            await events.autoSbcRefreshView(controller);
                            continue;
                        }
                    }
